...
# Ensure bot_path is defined
if [ -z "$bot_path" ]; then
    echo "Error: bot_path not defined"
    exit 1
fi
...
# Run bot
exec "$java_exe_path" --enable-preview "$@" -Dcaller_script_dir="$(pwd)" -cp "$bot_path" org.notarb.launcher.Main "--jupiter-arb-config-path=$config_path"
